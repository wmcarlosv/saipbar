...
class Common_model extends CI_Model {
    ...
    public function getAllOutlestByAssign() {
        ...
        if($role=="Admin"){
            $result = $this->db->query("SELECT * FROM tbl_outlets WHERE del_status='Live'")->result();
        }else{
            $result = $this->db->query("SELECT * FROM tbl_outlets WHERE FIND_IN_SET(`id`, '$outlets') AND del_status='Live'")->result();
        }
        return $result;
    }
    /**
