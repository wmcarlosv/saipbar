    ...
    public function ExcelDataAddIngredints() {
        ...
        if ($_FILES['userfile']['name'] != "") {
            if ($_FILES['userfile']['name'] == "Ingredient_Upload.xlsx") {
                ...
                if ($this->upload->do_upload('userfile')) {
                    ...
                    if ($totalrows > 2 && $totalrows < 54) {
                        ...
                        if ($arrayerror == '') {
                            ...
                            $this->session->set_flashdata('exception', 'Importe Completo!');
                            redirect('ingredient/ingredients');
                        } else {
                            unlink(FCPATH . 'asset/excel/' . $file_name); //File Deleted After uploading in database .
                        ...
                        unlink(FCPATH . 'asset/excel/' . $file_name); //File Deleted After uploading in database .
                        $this->session->set_flashdata('exception_err', "La carga es más de 50 o no se encontró ningun elemento.");
                    }
                } else {
                ...
                $this->session->set_flashdata('exception_err', "No podemos aceptar otros archivos, descargue el archivo de muestra 'Ingredient_Upload.xlsx', llénelo correctamente y cárguelo o cambie el nombre del archivo a 'Ingredient_Upload.xlsx' y luego llénelo.");
            }
        } else {
            $this->session->set_flashdata('exception_err', 'Archivo requerido');
        }
