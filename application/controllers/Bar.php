    ...
    public function update_cooking_status_ajax()
    {
        ...
        foreach($previous_id_array as $single_previous_id){
            ...
            if($cooking_status=="Started Cooking"){
                ...
                $notification = 'Item: '.$item_info->menu_name.' Esta listo para servir, Mesa: '.$tables_booked.', Cliente: '.$sale_info[0]->customer_name.', Order: '.$order_name;
     ...
     /**
     * update cooking status, delivery, Para Llevar
     * @access public
     * @return void
     * @param no
     */
            ...
            if($cooking_status=="Started Cooking"){
                ...
                if($i==$total_item){
                    ...
                    if($sale_info->order_type==1){
                        ...
                        $order_name = "B ".$sale_info->sale_no;
                        $order_type_operation = 'Orden de Delivery Lista para entregar';
                    }elseif($sale_info->order_type==3){
                        $order_name = "C ".$sale_info->sale_no;
                        $order_type_operation = 'Orden de Delivery está lista para entregar';
                    }
                    $notification = 'Cliente: '.$sale_info->customer_name.', Orden Numero: '.$order_name.' '.$order_type_operation;
                    $notification_data = array();        
                    $notification_data['notification'] = $notification;
                    $notification_data['sale_id'] = $sale_id;
                    $notification_data['outlet_id'] = $this->session->userdata('outlet_id');
