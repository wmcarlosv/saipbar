...
class Outlet extends Cl_Controller {
    ...
    public function addEditOutlet($encrypted_id = "") {
        ...
        $language_manifesto = $this->session->userdata('language_manifesto');
        $data_custom = getTotalLanuageManifesto();
        if(str_rot13($data_custom[0])=="eriutoeri"){
            if (file_exists(APPPATH.'controllers/Saas.php') && $data_custom[3]<=$data_custom[1]){
                redirect($data_custom[2]);
            }
        }
