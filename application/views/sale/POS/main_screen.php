...
foreach($food_menus as $single_menus){
    ...
    $menu_to_show .= '<p class="item_price">'.lang('price').': <span id="price_'.$single_menus->id.'">'.$single_menus->sale_price.'</span></p>';
...
foreach ($customers as $customer){
    $selected = "";
    $default_customer = $this->session->userdata('default_customer');
...
foreach ($waiters as $waiter){
    $selected = "";
    $default_waiter = $this->session->userdata('default_waiter');
...
foreach($tables as $table){
    $tables_modal .= '<div class="floatleft fix single_order_table by_room_'.$table->rooms_id.'" style="display:none;" id="single_table_info_holder_'.$table->id.'">';
    $tables_modal .= '<p class="table_name" class="ir_font_bold"><span id="sit_name_'.$table->id.'">'.$table->name.'<span></p>';
    $tables_modal .= '<p class="table_sit_capacity">Ordenes/Asientos: <span id="sit_capacity_number_'.$table->id.'">'.$table->sit_capacity.'<span></p>';
    ...
    $tables_modal .= '<p class="running_order_in_table">Ordernes en Mesa</p>';
    ...
    $tables_modal .= '<div class="floatleft fix column first_column">Orden</div>';
    $tables_modal .= '<div class="floatleft fix column second_column">Tiempo</div>';
    $tables_modal .= '<div class="floatleft fix column third_column">Personas</div>';
    $tables_modal .= '<div class="floatleft fix column forth_column">Borrar</div>';
    $tables_modal .= '</div>';
...
foreach($new_orders as $single_new_order){
    ...
    if($i==1){
        $order_list_left .= '<div data-started-cooking="'.$total_kitchen_type_started_cooking_items.'" data-done-cooking="'.$total_kitchen_type_done_items.'" class="txt_14 single_order fix" data-selected="unselected" id="order_'.$single_new_order->sales_id.'">';
    }else{
    ...
    $order_list_left .= '<p>Orden: <span class="running_order_order_number">'.$order_name.'</span></p>';
    $order_list_left .= '</div>';
...
foreach ($payment_methods as $payment_method){
    $selected = "";
    $default_payment = $this->session->userdata('default_payment');
...
<head>
    ...
    <base data-collect-tax="<?php echo escape_output($this->session->userdata('collect_tax')); ?>">
    </base>
    <base data-currency="">
    </base>
    <base data-role="<?php echo escape_output($this->session->userdata('role')); ?>">
    </base>
    <base data-collect-gst="<?php echo escape_output($this->session->userdata('tax_is_gst')); ?>">
    </base>
    ...
    <style type="text/css">
        ...
        div.room-container p{
            text-align: center;
        }

        #please_read_table_modal_button, #submit_table_modal, #proceed_without_table_button, #back_to_rooms, #new_or_order_number_table{
            display: none;
        }
    </style>
</head>
...
<body>
    ...
    <input type="hidden" id="delivery_not_possible_walk_in" value="<?php echo lang('delivery_not_possible_walk_in'); ?>">
    <input type="hidden" id="delivery_for_customer_must_address" value="<?php echo lang('delivery_for_customer_must_address'); ?>">
    ...
    <input type="hidden" id="default_customer_hidden" value="<?php echo escape_output($this->session->userdata('default_customer')); ?>">
    <input type="hidden" id="default_waiter_hidden" value="<?php echo escape_output($this->session->userdata('default_waiter')); ?>">
    ...
    <input type="hidden" name="last_invoice_id" class="last_invoice_id" id="last_invoice_id" value="<?php echo escape_output(getLastSaleId()) ?>">
        ...
        <div class="top_header_part">
            <div class="left_item">
                <div class="header_part_middle">
                    <ul class="icon__menu">
                        <li class="has__children">
                            ...
                            <ul class="sub__menu" role="menu">
                                <li><a href="<?php echo base_url();?>Authentication/userProfile"><?php echo lang('my_profile'); ?></a></li>
                                <li><a href="<?php echo base_url();?>Authentication/changePassword"><?php echo lang('change_password'); ?></a></li>
                                <li>
                        ...
                        <li class="has__children">
                            ...
                            <ul class="sub__menu" role="menu">
                                ...
                                foreach ($dir as $value):
                                    ...
                                    ?>
                                    <li class="<?=isset($language) && $language==$separete[1]?'active_lng':''?>"><a href="<?php echo base_url(); ?>Authentication/setlanguagePOS/<?=escape_output($separete[1])?>"><?php echo escape_output(ucfirst($separete[1]))?></a></li>
                                    <?php
                                endforeach;
                                ?>
                            </ul>
                        ...
                        <li>
                            <a href="#" id="open_hold_sales" class="header_menu_icon" data-tippy-content="<?php echo lang('open_hold_sale'); ?>">
                                <i class="fal fa-folder-open"></i>
                            </a>
                        </li>
                        <li><a href="javascript:void(0)" class="header_menu_icon" id="print_last_invoice" data-tippy-content="<?php echo lang('print_last_invoice'); ?>"><i class="fal fa-print"></i></a></li>
                        <li>
                            <a href="#" id="last_ten_sales_button" class="header_menu_icon" data-tippy-content="<?php echo lang('recent_sales'); ?>"><i class="fal fa-history"></i></a>
                        </li>
                        <li>
                            <a href="#" id="last_ten_feature_button" class="header_menu_icon" data-tippy-content="<?php echo lang('feature_sales'); ?>"><i class="fa fa-history"></i></a>
                        </li>
                        <li>
                        <a href="#" id="help_button" class="header_menu_icon" data-tippy-content="<?php echo lang('read_before_begin'); ?>"><i class="fal fa-question-circle"></i>
                        </a>
                        </li>
                        <li>
                        <a href="#" id="calculator_button" class="header_menu_icon" data-tippy-content="<?php echo lang('calculator'); ?>"> <i class="fal fa-calculator"></i>
                        </a>
                        </li>
                       
                    <li>
                        <a href="#" id="notification_button" class="header_menu_icon" data-tippy-content="<?php echo lang('kitchen_notification'); ?>">
                            <i class="fal fa-bell"></i> 
                            <span id="notification_counter" class="c_badge <?php echo escape_output($notification_number)?'':'txt_11'?>"><?php echo escape_output($notification_number); ?></span>
                        </a>
                    </li>
                    <li>
                        <a href="#" id="register_close" class="header_menu_icon" data-tippy-content="<?php echo lang('register'); ?>">
                        <i class="fal fa-registered"></i>
                            </a>
                    </li>
                    <li>
                        <a href="#" class="header_menu_icon" id="go_to_dashboard" 
                    ...
                    <li><a href="javascript:void(0)" id="fullscreen" class="header_menu_icon" data-tippy-content="<?php echo lang('fullscreen_1'); ?>"><i class="fal fa-expand-arrows-alt"></i></a></li>
                        <li>
                            <a href="#" data-tippy-content="<?php echo lang('main_menu'); ?>" id="open__menu" class="header_menu_icon">
            ...
            <div class="header_part_right">
                <ul class="btn__menu">
                    ...
                    <li><a href="#" data-status ="veg" class="veg_bev_item bg__green"><?php echo lang('vegetarian_items'); ?></a></li>
                    <li><a href="#" data-status ="bev" class="veg_bev_item bg__grey"><?php echo lang('beverage_items'); ?></a></li>
                </ul>
            </div>
        </div>
        <div class="top_header_for_mobile">
            <button type="button" data-isActive="false" id="show_running_order" class="bg__red">
            <i class="far fa-bags-shopping"></i> Ordenes Activas</button>
                
            <button type="button" id="show_cart_list" class="bg__purple">
            <i class="far fa-list-alt"></i> Lista</button>
            <button type="button" id="show_product" class="bg__grey">
                <i class="far fa-th"></i> Productos</button>
            <button type="button" id="show_all_menu" class="bg__green">
            <i class="fal fa-bars"></i> Menú</button>
        </div>
        <div id="main_part">
            <div class="left_item">
                <div class="main_left">
                    <div class="holder">
                        <div id="running_order_header">
                            ...
                            <input type="text" name="search_running_orders" id="search_running_orders" autocomplete='off'
                                class="ir_h15_m_w90" placeholder="<?php echo lang('customer_waiter_order_table'); ?>" />
                        </div>
                        ...
                        <div class="ir_pa_b_w100" id="left_side_button_holder_absolute">
                            ...
                            <button class="operation_button" id="modify_order"><i class="fas fa-edit"></i><?php echo lang('modify_order_'); ?></button>
                            <?php } ?>
                            ...
                            <div class="ir_flex_jc_pw94">
                                <button class="ir_calc_w98 operation_button fix btn_tip" id="print_kot" data-tippy-content="<?php echo lang('print'); ?> <?php echo lang('KOT'); ?>">
                                    <i class="fas fa-print"></i> <?php echo lang('KOT'); ?>
                                </button>
                                
                                <button class="operation_button ir_calc_w98_m5 btn_tip" id="print_bot" data-tippy-content="<?php echo lang('print'); ?> <?php echo lang('BOT'); ?>">
                            ...
                            <div class="ir_flex_jc_w94_pr">
                                <?php if($this->session->userdata('pre_or_post_payment') == "Post Payment"){?>
                                <button class="ir_calc_w98 operation_button fix" id="create_invoice_and_close">
                                    <?php echo lang('invoice'); ?>
                                </button>
                                
                                <?php } ?>
                                <button class="operation_button fix ir_calc_w98_m5 btn_tip" id="create_bill_and_close" data-tippy-content="<?php echo lang('Print_Bill_for_Customer_Before_Invoicing'); ?>">
                                    <?php echo lang('bill'); ?>
                                </button>
                            ...
                            <button class="operation_button fix" id="close_order_button"><i class="fas fa-times-circle"></i>
                                <?php echo lang('close_order'); ?></button>
                            <?php } ?>

                            <?php if($this->session->userdata('pre_or_post_payment') == "Post Payment"){?>
                            <button class="operation_button fix" id="cancel_order_button"><i class="fas fa-ban"></i>
                ...
                <div class="main_middle">
                    <div class="main_top fix">
                        ...
                        <div class="button_holder">
                                ...
                                <button data-selected="unselected" id="dine_in_button">
                                    <i class="fal fa-table"></i> <?php echo lang('rooms'); ?>
                                </button>
                            
                                <button id="take_away_button"><i class="fal fa-shopping-bag"></i>
                                    <?php echo lang('take_away'); ?></button>
                            
                                <button data-selected="unselected" id="delivery_button"><i
                                        class="fal fa-truck"></i> <?php echo lang('delivery'); ?></button>
                            
                                <button id="table_button"><i class="fal fa-table"></i> <?php echo lang('rooms'); ?></button>
                        ...
                        <div class="waiter_customer">
                            ...
                            <div class="separator">
                                <a href="#" data-tippy-content="<?php echo lang('edit_customer'); ?>" class="header_menu_icon" id="edit_customer">
                                    <i class="far fa-pencil-alt" ></i>
                                </a>
                                
                                <a href="#" id="plus_button" class="header_menu_icon" data-tippy-content="<?php echo lang('add_customer'); ?>">
                    ...
                    <div id="bottom_absolute">
                        <div class="bottom__info">
                            <div class="footer__content">
                                <div class="item">
                                    <input type="hidden" id="open_invoice_date_hidden" value="<?php echo date("Y-m-d",strtotime('today')) ?>">
                                    <div><?php echo lang('total_item'); ?>: <span id="total_items_in_cart_with_quantity">0</span>
                                        <i data-tippy-content="Invoice Date" class="fal fa-calendar-alt input-group date datepicker_custom calendar_irp" id="open_date_picker"></i>
                                    </div>
                                    <span id="total_items_in_cart" class="ir_display_none">0</span>
                                </div>
                                <div class="item">
                                    <span><?php echo lang('sub_total'); ?>:</span>
                                    <span id="sub_total_show">0.00</span>
                                    <span id="sub_total" class="ir_display_none">0.00</span>
                                    <span id="total_item_discount" class="ir_display_none">0</span>
                                    <span id="discounted_sub_total_amount" class="ir_display_none">0.00</span>
                                </div>
                                <div class="item">
                                    <span>
                                        <?php echo lang('discount'); ?>: <i class="fal fa-edit" id="open_discount_modal"></i> <span id="show_discount_amount">0.00</span>
                                    </span>
                                </div>
                                <div class="item">
                                ...
                                <div class="item">
                                    ...
                                    <i class="fal fa-eye" id="open_tax_modal"></i>
                                    <span id="show_vat_modal">0.00</span>
                                </div>
                                <div class="item">
                                    <span><?php echo lang('delivery_charge'); ?>: <i class="fal fa-edit" id="open_charge_modal"></i> <span id="show_charge_amount">0.00</span></span>
                                    
                                </div>
                              
                            </div>       
                            <div class="payable">
                                <h1><?php echo lang('total_payable'); ?> : <span id="total_payable">0.00</span></h1>
                            </div>
                            <div class="main_bottom">
                                <div class="button_group">
                                        ...
                                        <button  id="direct_invoice"><i class="fas fa-file-invoice"></i>
                                            <span
        ...
        <ul class="menu__list">
                        ...
                        <li class="it_has_children">
                            ...
                            <ul class="sub_menu" role="menu">
                                ...
                                <li class="<?=isset($language) && $language==$separete[1]?'active_lng':''?>"><a href="<?php echo base_url(); ?>Authentication/setlanguagePOS/<?=escape_output($separete[1])?>"><?php echo escape_output(ucfirst($separete[1]))?></a>
                                </li> <?php endforeach; ?>
                            </ul>
                        </li>
                    <li>
        ...
        <div class="modal-content">
           ...
           <div class="content">
                <div>
                    ...
                    <input type="text"  class="special_textbox" placeholder="<?php echo lang('value'); ?>"
           ...
           <div class="content">
                <div>
                    ...
                    <select id="charge_type" class="select2">
                        ...
                        <option value="service"><?php echo lang('service'); ?></option>
                    </select>
                </div>
                <div>
                    <label for="charge_amount"><?php echo lang('amount'); ?></label>
                    <input type="text" name="" autocomplete="off" class="special_textbox" onfocus="select();" placeholder="<?php echo lang('amount'); ?>"
                                                id="delivery_charge" />
                </div>
           </div>
           <div class="btn__box">
            ...
            <div class="ir_mrx5">
                <div class="section1 fix">
                    ...
                    <div class="sec1_inside" id="sec1_2"><i class="fal fa-minus" id="decrease_item_modal"></i>
                        <span id="item_quantity_modal">1</span> <i class="fal fa-plus"
                            id="increase_item_modal"></i>
                    </div>
                    <div class="sec1_inside" id="sec1_3"> <span
                            id="modal_item_price_variable" class="ir_display_none">0</span><span
                ...
                <div class="section2 fix">
                    ...
                    <div class="sec2_inside" id="sec2_2">  <span
                            id="modal_modifier_price_variable">0</span>
                ...
                <div id="modal_discount_section">
                    <p class="ir_fl_m_font_16" id="discount_txt_focus"><?php echo lang('discount'); ?> <i data-tippy-content="<?php echo lang('txt_err_pos_6'); ?>" class="fal fa-question-circle tooltip_modifier"></i></p><input type="text" name="" onfocus="select();" id="modal_discount"
    ...
    <div id="show_tables_modal2" class="modal display_none">
        ...
        <div class="modal-content" id="modal_content_show_tables2">
            ...
            <div class="select_table_modal_info_holder2 scrollbar-macosx">
                ...
                <?php echo $rooms_modal; ?>

            </div>
            <div class="bottom_button_holder_table_modal">
                <div class="left half">
                    <button id="back_to_rooms"><i class="fas fa-arrow-left"></i>
                        Volver a Salas</button>
                </div>
                <div class="left half">
                ...
                <div class="right half">
                    <button class="floatright" id="submit_table_modal" style="background-color: #8bc34a; color: #fff;"><?php echo lang('submit'); ?></button>
    ...
    <div id="show_sale_hold_modal" class="modal">
        ...
        <h1 class="main_header fix"><?php echo lang('hold_sale'); ?> <a href="javascript:void(0)" class="alertCloseIcon">
                    <i class="fal fa-times"></i>
                </a></h1>
            <div class="hold_sale_modal_info_holder">
                <div class="btn__box">
                    <button type="button" class="bg__red" data-selectedBtn="unselected" id="sale_hold_modal_order_details">Order Details</button>
                    <button type="button" class="bg__green" data-selectedBtn="selected" id="sale_hold_modal_order_list">Order List</button>
                </div>
                <div class="detail_hold_sale_holder">
                    <div id="sale_hold_modal_order_info_list" class="hold_sale_left">
                        <label>
                            <input type="text" id="search_hold_sale" placeholder="<?php echo lang('search_customer_name_or_mobile_number'); ?>">
                            <button><i class="far fa-search"></i></button>
                        </label>
                        <div class="hold_list_holder">
                            <div class="header_row">
                                <div class="first_column column"><?php echo lang('hold_number'); ?></div>
                                <div class="second_column column"><?php echo lang('customer'); ?> (<?php echo lang('phone'); ?>)</div>
                                    ...
                                    <div class="bottom_total_calculation_hold">
                                        <div class="single_row first">
                                            ...
                                            <div class="item">
                                                <span><?php echo lang('sub_total'); ?></span>
                                                <span
                                                    id="sub_total_show_hold">0.00</span>
                                                ...
                                                <span id="discounted_sub_total_amount_hold"
                                                    class="ir_display_none">0.00</span>
                                            </div>
                                            <div class="item">
                                        ...
                                        <div class="single_row forth">
                                            <div class="item">
                                                <span><?php echo lang('total_discount'); ?>: </span>
                                                <span id="all_items_discount_hold">0.00</span>
                                            </div>
                                            <div class="item">
                                                <span><?php echo lang('vat'); ?>: </span>
                                                <span id="all_items_vat_hold">0.00</span>
                                            </div>
                                            <div class="item">
    ...
    <div id="show_last_ten_sales_modal" class="modal">
        ...
        <h1 class="main_header fix"><?php echo lang('recent_sales'); ?>
            ...
            <div class="last_ten_sales_modal_info_holder fix">
                <div class="btn_box">
                    <button type="button" id="recent_sales_order_details" data-selectedBtn="unselected" class="bg__red">Order Details</button>
                    <button type="button" id="recent_sales_order_list" data-selectedBtn="selected" class="bg__green">Order List</button>
                </div>
                <div class="last_ten_sales_holder">
                    
                    <div id="recent_sales_order_info_list" class="hold_sale_left">
                        <label>
                            <input type="text" id="search_sales_custom_modal" placeholder="<?php echo lang('search_customer_name_or_mobile_number'); ?>">
                            <button><i class="far fa-search"></i></button>
                        </label>
                        <div class="hold_list_holder">
                            <div class="header_row">
                                ...
                                <div class="item_modifier_details fix">
                                    ...
                                    <div class="bottom_total_calculation_hold">
                                        <div class="single_row first">
                                            ...
                                            <div class="item">
                                                <span><?php echo lang('sub_total'); ?>: </span>
                                                <span id="sub_total_show_last_10">0.00</span>
                                                <span id="sub_total_last_10" class="ir_display_none">0.00</span>
                                                <span id="total_item_discount_last_10" class="ir_display_none">0.00</span>
                                                <span id="discounted_sub_total_amount_last_10" class="ir_display_none">0.00</span>
                                            </div>
                                            <div class="item">
                                                    <span><?php echo lang('discount'); ?> :</span>
                                        ...
                                        <div class="single_row forth">
                                            <div class="item">
                                                <?php echo lang('total_discount'); ?> : <span id="all_items_discount_last_10">0.00</span>
                                            </div>
                                            <div class="item">
                                                <?php echo lang('vat'); ?>: 
                                                <span id="recent_sale_modal_details_vat">0.00</span>
                                            </div>
                                            <div class="item">
                                    ...
                                    <?php echo lang('total_payable'); ?>: <span
                                                    id="total_payable_last_10">0.00</span>
                                    </h1>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="button_holder">
                                ...
                                <div class="single_button_holder">
                                    <button id="last_ten_delete_button"
                                        class="ir_font_capitalize"><?php echo lang('delete'); ?></button>
                                </div>
                                <div class="single_button_holder">
    ...
    <div id="generate_sale_hold_modal" class="modal">
        ...
        <div class="modal-content" id="modal_content_generate_hold_sales">
            ...
            <div class="section7 fix">
                <div class="sec7_inside" id="sec7_1"><button id="hold_cart_info"><?php echo lang('submit'); ?></button></div>
                <div class="sec7_inside" id="sec7_2"><button id="close_hold_modal"><?php echo lang('cancel'); ?></button></div>
            </div>
        </div>

    </div>

    <div class="cus_pos_modal" id="customModal">
                ...
                <div class="hold_sale">
                    <div class="left_item">
                        <label class="search__item">
                            <input type="text" id="search_future_custom_modal" placeholder="<?php echo lang('search_customer_name_or_mobile_number'); ?>">
                            <button><i class="far fa-search"></i></button>
                        </label>
                    ...
                    <div class="right_item">
                        ...
                        <div class="waiter_customer_table">
                                <div class="fix order_type"><span class="ir_font_bold"><?php echo lang('order_type'); ?>: </span><span id="last_10_order_type_"></span>
                            </div>
                        </div>
                        <div class="waiter_customer_table multiItem">
                            ...
                            <div class="customer"><span class="ir_font_bold"><?php echo lang('customer'); ?>: </span><span
                                        id="last_10_customer_name_"></span></div>
                            <div class="table">
                        ...
                        <div class="footer__details">
                            ...
                            <div class="txt__subtotal">
                                <span class="total__item"><?php echo lang('total_item'); ?>: <span class="total_items_in_cart_last_10_">0</span></span>
                                <p class="txt"> <?php echo lang('total_discount'); ?>: <span class="all_items_discount_last_10_">0.00</span></p>
                            </div>
                            <div class="txt__subtotal">
                                <span><?php echo lang('sub_total'); ?>: <span class="sub_total_show_last_10_">0.00</span></span>

                                <p class="txt"><?php echo lang('vat'); ?>: <span class="recent_sale_modal_details_vat_">0.00</span></p>
                            </div>
                            <div class="txt__subtotal">
                                <span class="discount"><?php echo lang('discount'); ?>: <span class="sub_total_discount_last_10_">0.00</span></span>
                                <p class="txt"><?php echo lang('charge'); ?>: <span class="delivery_charge_last_10_">0.00</span></p>
                            </div>
                        </div>
                        <h3 class="payable"><?php echo lang('total_payable'); ?>: <span class="total_payable_last_10_">0.00</span></h3>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="order_detail_modal" class="modal">
        ...
        <div class="modal-content" id="modal_content_sale_details">
                <h1 class="order_detail_title">
                        ...
                        <a href="javascript:void(0)" class="alertCloseIcon" id="order_details_close_button2"><i class="fal fa-times"></i></a>
                </h1>
            <div class="order_detail_modal_info_holder fix">
                            ...
                            <div class="bottom_total_calculation_hold">
                                ...
                                <div class="item">
                                    ...
                                    <div><?php echo lang('sub_total'); ?>:
                                        <span id="sub_total_show_order_details">0.00</span>
                                        <span id="sub_total_order_details" class="ir_display_none">0.00</span>
                                        <span id="total_item_discount_order_details" class="ir_display_none">0.00</span>
                                    ...
                                    <span
                                            id="sub_total_discount_order_details"></span><span
                               ...
                               <div class="item">
                                    <div>
                                        ...
                                        <span id="all_items_discount_order_details">0.00</span>
                                    </div>
                                    <div>
                                        <?php echo lang('vat'); ?>:
                                        <span id="all_items_vat_order_details">0.00</span>
                                    </div>
                                    <div>
                                        <?php echo lang('delivery_charge'); ?>:
                                        <span id="delivery_charge_order_details">0.00</span>
                                    </div>
                               </div>
                            </div>
                            <h1 class="modal_payable"><?php echo lang('total_payable'); ?> <span id="total_payable_order_details">0.00</span></h1>
                        </div>
                    </div>
                </div>
                <?php if($this->session->userdata('pre_or_post_payment') == "Pre Payment"){?>
                ...
                <div class="create_invoice_close_order_in_order_details" id="order_details_post_invoice_buttons">
                    <button id="order_details_create_invoice_close_order_button"><i class="fas fa-file-invoice"></i>
                        <?php echo lang('create_invoice_close'); ?></button>
                </div>
                <?php } ?>
                <div class="create_invoice_close_order_in_order_details">
                    <button id="order_details_print_kot_button"><i class="fas fa-file-invoice"></i>
    ...
    <div id="kitchen_status_modal" class="modal">
        ...
        <div class="modal-content" id="modal_kitchen_status_details">
            <h1 id="kitchen_status_main_header">
                ...
                <a href="javascript:void(0)" class="ir_top22_right0 alertCloseIcon"
                    id="kitchen_status_close_button2"><i class="fal fa-times"></i></a>
            </h1>
        ...
        <div class="modal-content" id="modal_finalize_order_details">
            ...
            <div class="fo_1 fix">
                ...
                <div class="half fix floatleft textright"><span
                        id="finalize_total_payable">0.00</span></div>
            </div>
            <div class="fo_2 fix">
            ...
            <div class="fo_3  fix">
                <div class="half fix floatleft textleft"><?php echo lang('given_amount'); ?> <i data-tippy-content="<?php echo lang('txt_err_pos_7'); ?>" class="fal fa-question-circle given_amount_tooltip"></i></div>
    ...
    <div id="notification_list_modal" class="modal">
        ...
        <div class="modal-content" id="modal_notification_list_details">
            <h1 id="modal_notification_header">
                ...
                <a href="javascript:void(0)" class="alertCloseIcon" id="notification_close2"><i class="fal fa-times"></i></a>
            </h1>
    ...
    <div id="kot_list_modal" class="modal">
        ...
        <div class="modal-content" id="modal_kot_list_details">
            <h1 id="modal_kot_header">
                <?php echo lang('kot'); ?>
                <a href="javascript:void(0)" class="ir_top5_right_10 alertCloseIcon" id="cancel_kot_modal2"><i class="fal fa-times"></i></a>
            </h1>
    ...
    <div id="bot_list_modal" class="modal">
        ...
        <div class="modal-content" id="modal_bot_list_details">
            <h1 id="modal_bot_header">
                <?php echo "BARRA"; ?>
                <a href="javascript:void(0)" class="ir_top5_right_10 alertCloseIcon" id="cancel_bot_modal2"><i class="fal fa-times"></i></a>
            </h1>
    ...
    <!-- Custom Scrollbar -->
    <script type="text/javascript" src="<?php echo base_url(); ?>assets/POS/js/lib/scrollbar/jquery.scrollbar.min.js"></script>
    ...
    $(document).ready(function(){
        $("div.room-container").click(function(){
            let id = $(this).attr("data-room-id");
            $(".single_order_table").hide();
            $(".by_room_"+id).show();
            $("#please_read_table_modal_button, #submit_table_modal, #proceed_without_table_button, #back_to_rooms, #new_or_order_number_table").show();
            $("#modal_content_show_tables2 h1").text("Mesas");
            $('div.room-container').hide();
        });

        $("#table_modal_cancel_button, #back_to_rooms").click(function(){
            $("#please_read_table_modal_button, #submit_table_modal, #proceed_without_table_button, #back_to_rooms, #new_or_order_number_table").hide();
