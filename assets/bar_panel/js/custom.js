...
$(document).ready(function(){
	...
	$(document).on('click','.items_holder .single_item',function(){
		...
		if(single_order.attr('data-order-type')=="Dine In"){
			...
			swal({
				title: 'Alerta!', 
				text: "Debe seleccionar todo para el pedido PARA LLEVAR y DELIVERY, ya que estos son paquetes!",
	...
	$(document).on('click','.start_cooking_button',function(){
		...
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			...
			if(previous_id!=''){
				let url = base_url+"Bar/update_cooking_status_delivery_take_away_ajax";
				$.ajax({
					url:url,
					method:"POST",
					data:{
	...
	$(document).on('click','#select_all_items',function(){
		if($('#order_details_holder .single_order[data-selected=selected]').attr('data-order-type')=='Dine In'){
			...
			swal({
				title: 'Alert',
				text: "No es necesario que seleccione o anule ningún producto para llevar o delivery, porque debe entregar todos los productos en un paquete",
                confirmButtonColor: '#b6d6f6' 
			});
		}
	...
	$.ajax({
		...
		success:function(response) {
			...
			for (let key in response) {
				...
				let total_bar_type_started_cooking_items = response[key].total_bar_type_started_cooking_items;
				let total_bar_type_done_items = response[key].total_bar_type_done_items;
				let splitted_width = (parseFloat(width)/parseFloat(total_bar_type_items)).toFixed(2);
				let percentage_for_started_cooking = (parseFloat(splitted_width)*parseFloat(total_bar_type_started_cooking_items)).toFixed(2);
				let percentage_for_done_cooking = (parseFloat(splitted_width)*parseFloat(total_bar_type_done_items)).toFixed(2);
				...
				if(total_bar_type_items!=total_bar_type_done_items){
					$order_list_left += '<div class="fix floatleft single_order" data-order-type="'+order_type+'" data-selected="'+selected_unselected+'" id="single_order_'+response[key].sales_id+'">';
	                    ...
	                    $order_list_left += '<div class="single_order_button_holder" id="single_order_button_holder_'+response[key].sales_id+'">';
	                        $order_list_left += '<button class="select_all_of_an_order" id="select_all_of_an_order_'+response[key].sales_id+'">Seleccionar Todo</button><button class="unselect_all_of_an_order" id="unselect_all_of_an_order_'+response[key].sales_id+'">Deseleccionar todo</button><button class="start_cooking_button" id="start_cooking_button_'+response[key].sales_id+'">Preparar</button><button class="done_cooking" id="done_cooking_'+response[key].sales_id+'">Listo</button>';    
	                    $order_list_left += '</div>';
	                $order_list_left += '</div>';
	            }
				i++;
