...
$(document).ready(function(){
	...
	$(document).on('change','#group_by_order_item',function(){
		...
		let menu_name = $("#group_by_order_item option[value='"+menu_id+"']").text();
	...
	$(document).on('click','.items_holder .single_item',function(){
		...
		if(single_order.attr('data-order-type')=="Dine In"){
			if($(this).attr('data-selected')=="selected"){
				...
				let single_order_selected_item = single_order.find('.single_item[data-selected="selected"]').length;
				...
				$(this).attr('data-selected','selected');
				if($(this).find('.single_item_cooking_status').html()=='Sin'){
			...
			swal({
				title: 'Aviso', 
				text: "Debe seleccionar todo para preparar el pedido!",
	...
	$(document).on('click','.start_cooking_button',function(){
		...
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			if($('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length>0){
				...
				let previous_id = '';
				let j = 1;
				let total_items = $('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length;
				...
				swal({
					title: "Alert!",
					text: "Selecione un producto para Cocinar!",
					confirmButtonColor: '#b6d6f6' 
				})	
			}
		}else{
			let previous_id = '';
			let j = 1;
			let total_items = $('#single_order_'+sale_id+' .items_holder .single_item').length;
			...
			if(previous_id!=''){
				let url = base_url+"Kitchen/update_cooking_status_delivery_take_away_ajax";
				$.ajax({
					url:url,
					method:"POST",
					data:{
	...
	$(document).on('click','.done_cooking',function(){
		...
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			if($('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length>0){
				...
				swal({
					title: "Alert!",
					text: "Por favor, seleccione un producto para cocinar!",
					confirmButtonColor: '#b6d6f6' 
				})	
			}
		}else{
	...
	$('#select_all_items').on('click',function(){
		if($('#order_details_holder .single_order[data-selected=selected]').attr('data-order-type')=='Dine In'){
			...
			swal({
				title: 'Alert',
				text: "No es necesario que seleccione o anule la selección de ningún Producto para llevar o entregar, porque debe entregar todos los artículos en una orden",
                confirmButtonColor: '#b6d6f6' 
			});
		}
	...
	$(document).on('click','#deselect_all_items',function(){
		if($('#order_details_holder .single_order[data-selected=selected]').attr('data-order-type')=='Dine In'){
			...
			swal({
				title: 'Alert',
				text: "No es necesario que seleccione o anule la selección de ningún Producto para llevar o entregar, porque debe entregar todos los productos en una orden",
                confirmButtonColor: '#b6d6f6' 
			});
			...
			success:function(response) {
				...
				for (let key in response.items) {
					...
					draw_table_for_order += '<div '+backgroundForSingleItem+' data-order-type="'+order_type+'" data-selected="unselected" class="single_item_in_order fix floatleft" id="single_item_'+this_item.previous_id+'">';
						...
						draw_table_for_order += '<p class="item_qty">Cant: '+this_item.qty+'</p>';
						draw_table_for_order += '<p class="modifier_name">Modificadores: '+selected_modifiers+'</p>'
						draw_table_for_order += '<p class="note">Nota: '+this_item.menu_note+'</p>';
	...
	$(document).on('click','#notification_remove_all',function(){
		...
		if($('.single_notification_checkbox:checked').length>0){
			...
			swal({
				title: 'Alert',
				text: 'No se selecciono ninguna notificación',
                confirmButtonColor: '#b6d6f6' 
			});
		}
	});
			...
			for (let key in response) {
				...
				notifications_list += '<div class="single_row_notification fix" id="single_notification_row_'+this_notification.id+'">';
					...
					notifications_list += '<div class="fix single_serve_button">';
						notifications_list += '<button class="single_serve_b" id="notification_serve_button_'+this_notification.id+'">Borrar</button>';
					notifications_list += '</div>';
				notifications_list += '</div>';
			}
	...
	$.ajax({
		...
		success:function(response) {
			...
			for (let key in response) {
				...
				let selected_unselected = ($('#selected_order_for_refreshing_help').html()==response[key].sales_id)?"selected":"unselected";
				let selected_background = ($('#selected_order_for_refreshing_help').html()==response[key].sales_id)?' style="background-color:#b6d6f6" ':'';
				let width = 100;
				let total_kitchen_type_items = response[key].total_kitchen_type_items;
				let total_kitchen_type_started_cooking_items = response[key].total_kitchen_type_started_cooking_items;
				let total_kitchen_type_done_items = response[key].total_kitchen_type_done_items;
				let splitted_width = (parseFloat(width)/parseFloat(total_kitchen_type_items)).toFixed(2);
				let percentage_for_started_cooking = (parseFloat(splitted_width)*parseFloat(total_kitchen_type_started_cooking_items)).toFixed(2);
				let percentage_for_done_cooking = (parseFloat(splitted_width)*parseFloat(total_kitchen_type_done_items)).toFixed(2);
				...
				let table_name = (response[key].table_name!=null)?response[key].table_name:"";
				let waiter_name = (response[key].waiter_name!=null)?response[key].waiter_name:"";
				let customer_name = (response[key].customer_name!=null)?response[key].customer_name:"";
				let booked_time = new Date(Date.parse(response[key].date_time));
				let now = new Date();
				
				let  days = parseInt((now - booked_time) / (1000 * 60 * 60 * 24));
				let  hours = parseInt(Math.abs(now - booked_time) / (1000 * 60 * 60) % 24);
				let  minute = parseInt(Math.abs(now.getTime() - booked_time.getTime()) / (1000 * 60) % 60);
				let  second = parseInt(Math.abs(now.getTime() - booked_time.getTime()) / (1000) % 60);
				minute = minute.toString();
				second = second.toString();
				minute = (minute.length==1)?'0'+minute:minute;
				second = (second.length==1)?'0'+second:second;
					...
					$order_list_left += '<div class="fix floatleft single_order" data-order-type="'+order_type+'" data-selected="'+selected_unselected+'" id="single_order_'+response[key].sales_id+'">';
	                    $order_list_left += '<div class="header_portion light-blue-background fix">';
	                        $order_list_left += '<div class="fix floatleft" style="width:70%;">';
	                            $order_list_left += '<p class="order_number">Orden: '+order_name+'</p> ';
	                            ...
	                            $order_list_left += '<p class="order_number">Mesa: '+tables_booked+'</p> ';
	                            ...
	                            $order_list_left += '<div data-selected="unselected" class="fix single_item '+item_background+'" data-order-id="'+response[key].sales_id+'" data-item-id="'+single_item.previous_id+'" id="detail_item_id_'+single_item.previous_id+'" data-cooking-status="'+single_item.cooking_status+'">';
	                                $order_list_left += '<div class="single_item_left_side fix">';
		                                ...
		                                $order_list_left += '<div class="fix floatleft item_detail">';
		                                    ...
		                                    $order_list_left += '<p class="item_qty" style="font-weight:bold; '+font_style+'">Cant: '+single_item.qty+'</p>';
	                    ...
	                    $order_list_left += '<div class="single_order_button_holder" id="single_order_button_holder_'+response[key].sales_id+'">';
	                        $order_list_left += '<button class="select_all_of_an_order" id="select_all_of_an_order_'+response[key].sales_id+'">Seleccionar Todo</button><button class="unselect_all_of_an_order" id="unselect_all_of_an_order_'+response[key].sales_id+'">Deselaccionar Todo</button><button class="start_cooking_button" id="start_cooking_button_'+response[key].sales_id+'">Cocinar</button><button class="done_cooking" id="done_cooking_'+response[key].sales_id+'">Listo</button>';    
	                    $order_list_left += '</div>';
	                $order_list_left += '</div>';
	            }
				i++;
			...
			*******************************************************************************************************************************************************************/
            let group_order_by_item_option = '<select id="group_by_order_item" class="group_by_order_item">';
